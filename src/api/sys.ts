 ...
 * @LastEditTime: 2022-02-24 16:24:16
 */
...
export const saveUserInfo = (userInfo: IUserInfo) =>
    Req.post(`${Prefix}/saveUserInfo`, userInfo);
// 保存菜单(新增、编辑)
export const saveMenu = (menuInfo: IMenu) =>
    Req.post(`${Prefix}/saveMenu`, menuInfo);
// 保存部門(新增、编辑)
export const saveDept = (deptInfo: IDept) =>
    Req.post(`${Prefix}/saveDept`, deptInfo);
// 保存角色(新增、编辑)
export const saveRole = (roleInfo: IRole) =>
    Req.post(`${Prefix}/saveRole`, roleInfo);
// 删除账号(删除单个、多个,逗号分割)
export const delUsers = (ids: string) =>
    Req.post(`${Prefix}/delUsers`, { ids });
// 删除菜单
