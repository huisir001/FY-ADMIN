 ...
 * @LastEditTime: 2022-02-24 16:46:52
-->
<template>
...
import useReqLogsOptions from './hooks/useReqLogsOptions'
import { getReqLogsByPage, delReqLogs } from '@/api/monitor'
import { ElMessage } from 'element-plus'

// 表格配置
const { searchOptions, tableCols, tableTools } = useReqLogsOptions()
// loading
...
const getLogs = (function getLogs() {
    loading.value = true
    getReqLogsByPage({ page: currPage.value, limit: limit.value, search: searchParams }).then(
        (res) => {
            const { ok, data } = res
            if (ok) {
...
const toolsBtnClick = async (btn: TOptionOfTools, flag: any) => {
    ...
    // 删除选定行
    if (btn === 'delete') {
        const { ok, msg } = await delReqLogs(flag.map((item: { id: string }) => item.id).join(','))
        if (ok) {
...
const handleTodo = async (btn: string, index: number, row: IObj) => {
    switch (btn) {
        // 删除按钮
        case 'del':
            const { ok, msg } = await delReqLogs(row.id)
            if (ok) {
                ElMessage.success(msg)
                getLogs()
            }
